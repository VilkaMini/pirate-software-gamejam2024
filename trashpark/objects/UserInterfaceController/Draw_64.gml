...
if (!global.game_running)
{
	...
	draw_set_colour(c_red);
	draw_text_transformed(camera_get_view_width(view_camera[0])/2, camera_get_view_height(view_camera[0])/2, "GAME OVER", 1, 1, 0);
}
...
if (global.near_dumpster)
{
	draw_set_halign(fa_center);
	draw_set_colour(c_yellow);
	draw_text_transformed(camera_get_view_width(view_camera[0])/2, camera_get_view_height(view_camera[0])/1.5, "PRESS SPACE TO DEPOSIT", 1, 1, 0);
}

draw_set_halign(fa_center);
draw_set_colour(c_yellow);
draw_text_transformed(camera_get_view_width(view_camera[0])/2, 10, string_concat("SCORE: ", global.total_score), 1, 1, 0);
